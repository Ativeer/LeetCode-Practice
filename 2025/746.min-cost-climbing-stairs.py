...
class Solution:
    def minCostClimbingStairs(self, cost: List[int]) -> int:
        ...
        '''
        use of recurssion more than backtracking
        Solution goes TLE.
        for the test case:
        ...
        #     if curr_pos == 0 or curr_pos == 1: return cost[curr_pos]
        #     # include the cost for this step, and minimum between the next step included and excluded
        #     return cost[curr_pos] + min(backtrack(curr_pos-1), backtrack(curr_pos-2))
        # return min(backtrack(len(cost)-1), backtrack(len(cost)-2))

        '''
        Use Memoization
        we can skip repeated recurssion calls by seeing if the recussion is already performed or not.
        '''
        
        # def backtrack(curr_pos, memo):
        #     # base cases
        #     if curr_pos < 0: return 0
        #     if curr_pos == 0 or curr_pos == 1: return cost[curr_pos]
        #     if memo[curr_pos] == -1:
        #         memo[curr_pos] = cost[curr_pos] + min(backtrack(curr_pos-1, memo),
        #                                             backtrack(curr_pos-2, memo))
        #     return memo[curr_pos]
        # return min(backtrack(len(cost)-1, [-1 for _ in range(len(cost))]),
        #            backtrack(len(cost)-2, [-1 for _ in range(len(cost))]))
    
        '''
        DP
        Update the recussion calls into a table/array/stack.
        '''
        # dp = [i for i in cost]
        # for val in range(2, len(dp)):
        #     dp[val] = cost[val] + min(dp[val-1], dp[val-2])
        # return min(dp[-1], dp[-2])

        '''
        Space optimized
        since we need only the last two values, we can remove the rest of the array.
        '''
        first = cost[0]
        second = cost[1]
        for val in range(2, len(cost)):
            curr = cost[val] + min(first, second)
            first = second
            second = curr
